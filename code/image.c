...
{
     12,12,11.5,11.5,11.5,11.5,11,11,11,11,                       //0-9琛�
...
//三极管标志位
int16_t transistor_b_falg = 0;
int16_t transistor_c_falg = 0;
int16_t transistor_e_falg = 0;
...
//      }
    ...
    if (right_end_flag == 1)//锟斤拷锟斤拷冶呓锟斤拷锟窖碉拷锟斤拷 锟斤拷始锟斤拷锟斤拷锟揭憋拷锟斤拷
    {
        for (uint8 i = right_end - 1; i > 0; i--)
        {
            ...
            {
                if (image_two_value[i][j] == black_point && image_two_value[i][j - 1] == white_point && image_two_value[i][j-2]==white_point)//锟揭碉拷锟斤拷一锟斤拷锟斤拷色
                {
                    right_line_list[i] = j;
                    break;
                }
            ...
            if (right_line_list[i] == -1 && right_line_list[i + 1] != -1 && i > 20)//锟斤拷锟竭诧拷锟斤拷
            {
                ...
                for (uint8 jump = i; jump > jump_end; jump -= step)
                {
                    ...
                    for (uint8 j = start_target; j > end_target; j--)
                    {
                        ...
                        if (image_two_value[jump][j] == black_point && image_two_value[jump][j - 1] == white_point&& image_two_value[jump][j-2]==white_point)//锟揭碉拷锟斤拷一锟斤拷锟斤拷色
                        {
                            junp_found = j;
                            break;
                        }
    ...
    if (left_end_flag == 1)
    {
        for (uint8 i = left_end - 1; i > 0; i--)
        {
            ...
            if (left_line_list[i] == -1) break;//全锟斤拷锟斤拷锟疥还锟斤拷没锟窖碉拷 锟斤拷锟斤拷
        }
    }
...
void stright_angle(void)
{
  ...
  if(Benzene_turn_flag_left==1 && Benzene_turn_flag_right==0 && Benzene_turn_flag_up==0)  //左直角
    ...
    else if(Benzene_turn_flag_left== 0 && Benzene_turn_flag_right== 1 && Benzene_turn_flag_up==0 &&cross_flag==0) //右直角
    {
...
void T_corner_h(void)
{
    ...
    if(Benzene_turn_flag_up>=1 && Benzene_turn_flag_left==1 && Benzene_turn_flag_right==0 && cross_flag==0 && Benzene_turn_point_up1 > 20 && Benzene_turn_point_up1 < MT9V03X_W - 30)//T澶存湞鍙�
    {
      left_T_h_flag=1;
    }
    else if(Benzene_turn_flag_up>=1 && Benzene_turn_flag_left==0 && Benzene_turn_flag_right==1 && cross_flag==0 && Benzene_turn_point_up1 > 20 && Benzene_turn_point_up1 < MT9V03X_W - 30)//T澶存湞宸�
    {
      right_T_h_flag=1;
    }
    else
