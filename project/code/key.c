...
void KEY_INIT(void)
{
   gpio_init(P11_0, GPI, 1, GPO_PUSH_PULL);
   gpio_init(P22_0, GPI, 1, GPO_PUSH_PULL);
   gpio_init(P23_3, GPI, 1, GPO_PUSH_PULL);
   gpio_init(P23_4, GPI, 1, GPO_PUSH_PULL);
}

uint8_t Key_Get(void)
{
  uint8_t  KEY_VALUE = 0;
  if(gpio_get_level(P11_0) == 0)
  {
      system_delay_ms(40);
      if(gpio_get_level(P11_0) == 0)
      {
  ...
  else if(gpio_get_level(P22_0) == 0)
  {
   system_delay_ms(40);
  ...
  else if(gpio_get_level(P23_3) == 0)
  {
    system_delay_ms(40);
      if(gpio_get_level(P23_3) == 0)
      {
        KEY_VALUE = 3;
      }
  }
  else if(gpio_get_level(P23_4) == 0)
  {
      system_delay_ms(40);
      if(gpio_get_level(P23_4) == 0)
      {
        KEY_VALUE = 4;
      }
  }
...
void PID(void)
{
  ...
  para_encoder += left_encoder;
   ...
   ips200_show_char(180,last_select*20,' ');//����ϴ�����
   ips200_show_char(180,select*20,'*');//��ʾ�������
...
//  }  
  ...
  ips200_show_string(0,240,"write_flash");    // 添加写入flash选项
  ...
  if(down_encoder>100)
  {
    down_encoder=0;
    select++;
    if(select>13) select=0;  // 增加选项数量
  }
  else if(down_encoder<-100)
  {
    down_encoder=0;
    select--;
    if(select<0) select=13;  // 增加选项数量
  }
  ...
  if(key == 1)
  {
    if(select==12)  // write_flash选项
    {
      Write_Flash();
    }
    else if(select==13)  // back选项
    {
      menu_page = 0;
      select = 2;     // 返回主菜单时设置到image选项
      last_select = 2;
      down_encoder=0;
      para_encoder=0;
    }
...
void menu()
{
  static uint8_t last_menu_page = 0;  // 记录上一次的菜单页面
  ...
  if(menu_page == 0)
  {
    ...
    ips200_show_char(180,last_select*20,' ');//清除上次索引
    ips200_show_char(180,select*20,'*');//显示这次索引
...
void Write_Flash(void)
{
  ...
  flash_write_page_from_buffer(0, write_flash, 50);
}

void Read_flash(void)
{
  flash_read_page_to_buffer(0, read_flash, 50);
